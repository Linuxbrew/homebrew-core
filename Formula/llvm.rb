...
class Llvm < Formula
  ...
  unless OS.mac?
    ...
    depends_on "binutils" if OS.linux? # needed for gold plugin and stripping executables/libraries/object files
  ...
  def install
    ...
    system "strip", *(Dir[bin/"*", lib/"*"]).select { |f| Pathname.new(f).file? && Pathname.new(f).elf? } unless OS.mac?
  end
